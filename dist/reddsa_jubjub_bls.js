...
import { poseidon as poseidonFactory, validateOpts, } from '@noble/curves/abstract/poseidon';
...
const t4json = JSON.parse(readFileSync(join(__dirname, '../poseidon_consts_t4.json'), 'utf8'));
// ---- Curve constants ----
...
const Fr = bls12_381.Fr ?? bls12_381.fields?.Fr;
if (!Fr)
    throw new Error('Could not find bls12_381.Fr');
...
const map2DBig = (M) => M.map((row) => row.map(toBig));
const poseidonF4 = poseidonFactory(validateOpts({
    Fp: Fr,
    t: t4json.t, // 4
...
export function generateSecretSeed() {
    ...
    // avoid s==0
    if (bytesToNumberBE(sk) % n === 0n)
        sk[0] ^= 1;
    return sk;
}
export function getPublicKey(s) {
    if (s === 0n)
        s = 1n;
...
function deriveKeysFromSeed(skBytes) {
...
export function signPoseidon(msg, skSeed) {
    ...
    if (r === 0n) {
        ...
        if (r === 0n)
            throw new Error("nonce derivation failed");
    }
...
export function verifyPoseidon(msg, pk, sig) {
    try {
        const R = jubjub.Point.fromHex(bytesToHex(sig.R));
        if (R.is0())
            return false;
        const A = jubjub.Point.fromHex(bytesToHex(pk));
        if (A.is0() || A.isSmallOrder() || !A.isTorsionFree?.())
            return false; // or [n]A==0
        // parse S
